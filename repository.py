...
class Repository:
    ...
    def __init__(self, showLog=False):
        ...
        self.__engine = create_engine(
            'postgres://{}:{}@localhost:{}/{}'.format(DATABASE['USER'], DATABASE['PASSWORD'], DATABASE['PORT'],
                                                      DATABASE['DB_NAME']), echo=self.__showLog, pool_size=20,
            pool_recycle=3600)
    ...
    def populate_data(self):
        locations = ['Addis Ababa', 'Gondar', 'Bahir Dar', 'Jimma']
        for i in range(100):
    ...
    def fragment(self, relation, predicate):
        result = self.__engine.connect().execute(
            text("select * from " + relation + " where " + predicate.attribute + predicate.operation + "'" + str(
                predicate.value) + "'")
        )
